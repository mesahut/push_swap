...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/22 16:44:31 by mayilmaz          #+#    #+#             */
/*   Updated: 2025/01/22 18:19:19 by mayilmaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list	*ft_less_cost(t_list **head_a, t_list **head_b)
{
	...
	pla_a = 1;
	cost_node = (*head_a);
	cost = ft_count_cost(head_b, cost_node->index, 0);
	less = cost_node;
	cost_node = cost_node->next;
	while (cost_node)
	{
		newcost = ft_count_cost(head_b, cost_node->index, pla_a);
		if (newcost < cost)
		{
			less = cost_node;
			cost = newcost;
	...
	while (top_b)
	{
		if (index_a > top_b->index)
		{
			if (!closest_smaller_node
				|| (top_b->index > closest_smaller_node->index))
			{
